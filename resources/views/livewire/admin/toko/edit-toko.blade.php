<div class="container mx-auto p-6 bg-white rounded-lg shadow-md">
    ...
    <form wire:submit.prevent="update">
        ...
        <div class="mb-4">
            <label for="no_wa" class="block text-sm font-medium text-gray-700 mb-1">Link Wa</label>
            <input
                type="text"
                id="no_wa"
                wire:model="no_wa"
        ...
        <div class="space-y-6">
            <!-- Grid Gambar yang Ada -->
            <div>
                <h3 class="text-lg font-medium text-gray-800 mb-4">Gambar yang Sudah Ada</h3>
                <div class="grid grid-cols-3 gap-6">
                    @foreach ($existingImages as $key => $image)
                        <div class="relative group bg-gray-100 rounded-lg shadow-md overflow-hidden">
                            <img 
                                src="{{ asset('storage/images/' . $image) }}" 
                                alt="Gambar Produk" 
                                class="w-full h-32 object-cover transition-transform transform group-hover:scale-105"
                            >
                            <button 
                                wire:click.prevent="removeImageFromUpdate('{{ $image }}')" 
                                class="absolute top-2 right-2 bg-red-500 text-white px-3 py-1 text-xs font-medium rounded-full shadow opacity-0 group-hover:opacity-100 transition-opacity"
                            >
                                Hapus
                            </button>
                        </div>
...
<script>
    ...
    $(document).ready(function() {
        $('#cover-image').on('change', function() {
            ...
            // Jika jumlah file lebih dari 3, tampilkan pesan error
            if (files.length > maxFiles) {
                errorMessage.show();  // Menampilkan pesan kesalahan
                // Reset input file
                $(this).val('');
            } else {
                errorMessage.hide();  // Menyembunyikan pesan kesalahan
            }
        });
